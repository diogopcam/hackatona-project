...
class SegmentedControl: UIView {
    ...
    private let segmentedControl: UISegmentedControl = {
        let control = UISegmentedControl(items: ["Employes", "Locals", "Events"])
        control.selectedSegmentIndex = 0

        control.backgroundColor = UIColor(named: "background-secondary") ?? .systemGray6
...
extension SegmentedControl: ViewCodeProtocol {
    ...
    func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            segmentedControl.bottomAnchor.constraint(equalTo: self.safeAreaLayoutGuide.bottomAnchor),
